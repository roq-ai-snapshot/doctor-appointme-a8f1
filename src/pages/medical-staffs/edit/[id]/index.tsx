...
import { useRoqClient, useMedicalStaffFindFirst } from 'lib/roq';
...
function MedicalStaffEditPage() {
  ...
  const { data, error, isLoading, mutate } = useMedicalStaffFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: MedicalStaffInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.medical_staff.update({
        data: values as RoqTypes.medical_staff,
        where: {
          id,
        },
      });
